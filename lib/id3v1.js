...
function parse( buf ) {
  ...
  const info = {
    title: readBytesToUtf8( meta.slice( TITLE_OFFSET, TITLE_OFFSET + 30 ) ),
    artist: readBytesToUtf8( meta.slice( ARTIST_OFFSET, ARTIST_OFFSET + 30 ) ),
    album: readBytesToUtf8( meta.slice( ALBUM_OFFSET, ALBUM_OFFSET + 30 ) ),
    year: readBytesToUtf8( meta.slice( YEAR_OFFSET, YEAR_OFFSET + 4 ) ),
    comment: "",
    track: 0,
    genre: "unknown"
  };
  // 是否有音轨
  if ( meta[ 125 ] === 0 ) {
    info.comment = readBytesToUtf8( meta.slice( COMMENT_OFFSET, 125 ) );
    info.track = meta[ 126 ];
  } else {
    info.comment = readBytesToUtf8( meta.slice( COMMENT_OFFSET, 127 ) );
  }
